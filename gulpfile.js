...
gulp.task('copyFontAwesomeFonts', () => {
    return gulp.src('./node_modules/@fortawesome/fontawesome-free/webfonts/*').pipe(gulp.dest('./src/css/webfonts'));
});
...
gulp.task('setTargetEnvironment', (cb) => {
    ...
    if (params.length !== 1) throw new Error('Task "serve" must be followed by the parameter "--target=xyz"');
